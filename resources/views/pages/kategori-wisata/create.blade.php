...
@section('content')
    <div class="card">
        ...
        <div class="card-body">
            <form action="{{ route('spk/destinasi/kategori/wisata.store') }}" method="POST">
                @csrf
                <div class="row">
                    <div class="mb-3">
                        <label for="nama-kategori-wisata" class="form-label">Nama Kategori Wisata</label>
                        <input type="text" class="form-control form-control-md" id="nama-kategori-wisata" name="name" placeholder="Nama Kategori Wisata" value="{{ old('name') }}" required />
                    </div>
